...
void *readtx(void *arg){
  struct param *node = (struct param*)arg;// get tid and objno and count
   ...
   start_operation(node->tid, node->count);
      zgt_p(0);       // transaction manager is locked

       zgt_tx *currentTx = get_tx(node->tid);		// checking the currrent tranaction if it is holding the object lock

        if(currentTx != NULL && currentTx->status != 'A'){		// current transaction is not in active state

          currentTx->print_tm();
            
            zgt_hlink *obj_ptr;
            obj_ptr=ZGT_Ht->find(currentTx-> sgno,node->obno);		// checking whether object is in the hash table

             if(currentTx->status == TR_ACTIVE){			//Transaction is active
               zgt_v(0);       // tranaction manager is released

              
              int lockAvail = currentTx->set_lock(node->tid, currentTx-> sgno, node->obno, node->count, 'S', obj_ptr );		// checking whether lock is available for read

              
              if(lockAvail == 1)
                currentTx->perform_readWrite(currentTx-> tid,node->obno,'S');		// performing read operation in shared mode
            }


        }else{
           zgt_v(0);       // transaction manager is released
            
            fprintf(logfile, "\t Transaction %d doesn't exist or aborted. \n", node->tid);      // Write the log record and close
            fflush(logfile);
        }
        finish_operation(node->tid);
        pthread_exit(NULL);
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  start_operation(node->tid, node->count);
        ...
        zgt_tx *currentTx = get_tx(node->tid);		// check the current transaction if it holds the object lock

         if(currentTx != NULL && currentTx->status != 'A'){
            
            zgt_hlink *obj_ptr;
            obj_ptr=ZGT_Ht->find(currentTx-> sgno,node->obno);		// check whether object is in the hash table

            if(currentTx->status == TR_ACTIVE){		// transaction is active
              zgt_v(0);       // transaction manager is released

              
              int lockAvail = currentTx->set_lock(node->tid, currentTx-> sgno, node->obno, node->count, 'X', obj_ptr);		//check whether lock is available for the current transaction

              
              if(lockAvail == 1)
                currentTx->perform_readWrite(currentTx-> tid, node->obno, 'X');		// performing write operation in exclusive mode
            }

        }else{			// transaction does not exist
           zgt_v(0);       // transaction manager is released
            
            fprintf(logfile, "\t Transaction %d doesn't exist or aborted. \n", node->tid); // Write the log record and close the file
            fflush(logfile);
        }
...
void *aborttx(void *arg)
{
  ...
  pthread_exit(NULL);			// exit operation on the thread
}
 ...
 //remove the locks before committing
  struct param *node = (struct param*)arg;// get tid and count
   ...
   start_operation(node->tid, node->count);
        ...
        currentTx->status = 'E';  // status of the current trasaction is changed to end

        printf("Semno-> inside committx :: %d\n", currentTx->semno);

        do_commit_abort(node->tid,'E');

        zgt_v(0);       // release the transaction manager
        finish_operation(node->tid);
...
void *do_commit_abort(long t, char status){
  ...
  // write your code
        ...
        if(currentTx != NULL){
            ...
            currentTx->free_locks();		// locks held by the current transction on the object are free
          
            zgt_v(currentTx->tid);
          //transation node is removed from the transaction manager
            int check_semno = currentTx->semno;
            currentTx->end_tx();		// current transaction is ended

            

          
            if(check_semno > -1){
              int totalTransaction = zgt_nwait(check_semno); // total no of transactions waiting on the current tranaction to release the lock on a object
              printf("totalTranaction:: %d currentTx->Semno %d \n", totalTransaction, check_semno);

              if(totalTransaction > 0){
                for (int i = 0; i < totalTransaction; ++i)
                {   zgt_v(check_semno);   } 		// semaphores of waiting transactions are released

                totalTransaction = zgt_nwait(check_semno); 		// checking if there are any waiting transactions
                printf("totalTransaction::%d currentTx->Semno %d \n", totalTransaction, check_semno);
              }
            }else{
              printf("\ncheck_semno is -1\n");
            }
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1, zgt_hlink *obj_ptr){
  ...
  bool lock = false;
        ...
        while(!lock){
           //Check if it is a read operation or a write operation
           zgt_p(0);		// transaction manager is locked
           zgt_tx *currentTx = get_tx(tid1);

           if(obj_ptr == NULL){			// object does not exist in the hash table
              
              ZGT_Ht->add(currentTx, currentTx->sgno, obno1, currentTx->lockmode);		// insert the object and grant the lock to the current transaction

              currentTx->status = 'P';  //status of the current transaction is set to active
              
              zgt_v(0);       // transaction manager is released
              return(1);

            }else{			// transaction has lock on the object
                 ...
                 if(obj_ptr->tid == tid1){
                    ...
                    return(1);
                     ...
                     zgt_tx *nextTx = get_tx(obj_ptr->tid);  //Transaction which is holding the object
                     zgt_hlink *anotherTx = others_lock(obj_ptr, sgno1, obno1);  //Other transactions are waiting for the lock on the object

                     if(currentTx->Txtype=='R' && nextTx->Txtype=='R' && anotherTx->lockmode!='X'){
                      //Check if the current transaction and the transaction which is holding object have shared read.
                      //check if any other transactions are waiting on this object for a write operation.
                        lock = true;  //lock is granted

                        if(currentTx->head == NULL){
                          
                            ZGT_Ht->add(currentTx, currentTx->sgno, obno1, currentTx->lockmode);	// add the current transaction to an object in hash table
                            obj_ptr=ZGT_Ht->find(currentTx-> sgno, obno1);
                            
                            currentTx->head = obj_ptr;            //Point the head to the new object node
                            currentTx->status = 'P';		// status of the current transaction is set to active
                          ...
                          while(temp->nextp != NULL){		// iterated till the end of the object list
                            temp = temp->nextp;
                          }
                          temp->nextp = obj_ptr;              // nextp of the last object node is pointed to the new node pointer

                          currentTx->status = 'P';			//current transaction is set to active
                          
                          zgt_v(0);							// transaction manager is released
                          return(1);
                        }
                      }
                      else{									// not a shared mode transaction
                          
                           currentTx->status = 'W';         // status of the current transaction is set to wait
                           currentTx->obno = obno1;
                           currentTx->lockmode = lockmode1;
                           if(get_tx(obj_ptr->tid))
                           currentTx->setTx_semno(obj_ptr->tid, obj_ptr->tid);		// semaphore is set on the current transaction
                                                                             
                           else{
                            
                             currentTx->status = 'P';		// lock is granted and transaction is set to active
                             zgt_v(0);						// transaction manager is released
                             return(1);
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  zgt_p(0);        // transaction manager is locked
        ...
        ZGT_Sh->objarray[obno]->value--;		// object value is decremented by 1
         ...
         else if(lockmode == 'X'){ 		//Read-Write mode or an exclusive mode
         
          ZGT_Sh->objarray[obno]->value++;		//object value is incremented by 1
